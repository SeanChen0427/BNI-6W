...
### 修改
- PPT 提示詞重構為 RCTF 四段結構（Role / Context / Task / Format）
- Role：定義 AI 為資深簡報設計顧問，熟悉 BNI 引薦文化
- Context：帶入簡報者、行業、主題、投影片總數與場合說明
- Task：四點具體指令（版面配置、視覺重點、口說提示、投影片清單）
- Format：明確限制輸出格式，含字數上限、禁止 Emoji、清單格式
- 投影片內容改用 Markdown 語法：`##` 標題、`**bold**` 欄位名、表格呈現故事四格與三級引薦
- 三級引薦改為 Markdown 表格（層級 / 對象描述），暗示三欄版面意圖
